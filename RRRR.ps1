...
do {
    ...
    switch ($choice.ToUpper()) {
        "1" {
            ...
            # ===== Network Adapter (รันนอก Start-Job) =====
            $adapterOut = Get-NetAdapter | Where-Object { $_.Status -eq "Up" } | Select-Object -First 1
            if ($adapterOut) {
                $regPathOut = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}"
                $adapterKeyOut = Get-ChildItem $regPathOut -ErrorAction SilentlyContinue | Where-Object {
                    (Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue).NetCfgInstanceId -eq $adapterOut.InterfaceGuid
                }
                if ($adapterKeyOut) {
                    Set-ItemProperty -Path $adapterKeyOut.PSPath -Name "*TransmitBuffers" -Value "64" -Type String -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $adapterKeyOut.PSPath -Name "*JumboPacket" -Value "9014" -Type String -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $adapterKeyOut.PSPath -Name "*WakeOnMagicPacket" -Value "0" -Type String -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $adapterKeyOut.PSPath -Name "*WakeOnPattern" -Value "0" -Type String -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $adapterKeyOut.PSPath -Name "*ReceiveBuffers" -Value "32" -Type String -Force -ErrorAction SilentlyContinue
                }
            }
